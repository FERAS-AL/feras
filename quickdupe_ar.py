...
class QuickDupeApp:
    ...
    def build_ui(self):
        ...
        try:
            if hasattr(self, 'icon_image'):
                ...
                self.title_icon = small_icon
                icon_label = tk.Label(title_bar, image=small_icon, bg=self.colors['bg_light'])
                icon_label.pack(side='right', padx=(2, 8)) # يمين
                icon_label.bind('<Button-1>', start_drag)
                icon_label.bind('<B1-Motion>', drag)
        except:
            pass

        # عنوان التطبيق (بجانب الأيقونة يمين)
        title_label = tk.Label(title_bar, text="الناسخ السريع", bg=self.colors['bg_light'],
                               fg=self.colors['text'], font=('Segoe UI', 10, 'bold'))
        title_label.pack(side='right', padx=2)
        ...
        def update_scrollbar(*args):
            if len(args) == 2:
                ...
                thumb_height = max(30, thumb_bottom - thumb_top)
        ...
        self.canvas_window = self.canvas.create_window((0, 0), window=self.scrollable_frame, anchor='ne') # anchor NE for right alignment
        ...
        # Scrollbar LEFT, Canvas RIGHT (RTL)
        self.scrollbar_canvas.pack(side='left', fill='y')
        ...
        ttk.Checkbutton(frame, text="الإبقاء في المقدمة", variable=self.stay_on_top_var, command=self.toggle_stay_on_top).pack(anchor='e', padx=10, pady=5)

        self.show_overlay_var = tk.BooleanVar(value=self.config.get("show_overlay", True))
        ttk.Checkbutton(frame, text="إظهار النص على الشاشة", variable=self.show_overlay_var, command=self.save_settings).pack(anchor='e', padx=10, pady=5)

        # Variance
        variance_frame = ttk.Frame(frame)
        variance_frame.pack(fill='x', padx=10, pady=5)
        ttk.Label(variance_frame, text="تباين التوقيت:").pack(side='right')
        ...
        self.dc_both_hotkey_entry.pack(side='right', padx=5)
        self.dc_both_record_btn = ttk.Button(dc_both_frame, text="تعيين", width=6, command=self.start_recording_dc_both)
        self.dc_both_record_btn.pack(side='right')

        # DC Outbound
        dc_out_frame = ttk.Frame(frame)
        dc_out_frame.pack(fill='x', padx=10, pady=2)
        self.dc_outbound_btn = tk.Button(dc_out_frame, text="فصل الصادر", width=12, bg=self.colors['bg_light'], fg=self.colors['text'],
                                         activebackground=self.colors['highlight'], activeforeground='white', bd=0,
                                         command=self.toggle_dc_outbound)
        self.dc_outbound_btn.pack(side='right')
        ttk.Label(dc_out_frame, text="الاختصار:").pack(side='right', padx=(0, 10))
        ...
        self.dc_outbound_hotkey_entry = tk.Entry(dc_out_frame, textvariable=self.dc_outbound_hotkey_var, width=10, state="readonly",
                                                bd=0, highlightthickness=0, bg=self.colors['bg_light'], fg=self.colors['text'], readonlybackground=self.colors['bg_light'])
        self.dc_outbound_hotkey_entry.pack(side='right', padx=5)
        self.dc_outbound_record_btn = ttk.Button(dc_out_frame, text="تعيين", width=6, command=self.start_recording_dc_outbound)
        self.dc_outbound_record_btn.pack(side='right')

        # DC Inbound
        dc_in_frame = ttk.Frame(frame)
        dc_in_frame.pack(fill='x', padx=10, pady=2)
        self.dc_inbound_btn = tk.Button(dc_in_frame, text="فصل الوارد", width=12, bg=self.colors['bg_light'], fg=self.colors['text'],
                                        activebackground=self.colors['highlight'], activeforeground='white', bd=0,
                                        command=self.toggle_dc_inbound)
        self.dc_inbound_btn.pack(side='right')
        ttk.Label(dc_in_frame, text="الاختصار:").pack(side='right', padx=(0, 10))
        ...
        self.dc_inbound_hotkey_entry = tk.Entry(dc_in_frame, textvariable=self.dc_inbound_hotkey_var, width=10, state="readonly",
                                               bd=0, highlightthickness=0, bg=self.colors['bg_light'], fg=self.colors['text'], readonlybackground=self.colors['bg_light'])
        self.dc_inbound_hotkey_entry.pack(side='right', padx=5)
        self.dc_inbound_record_btn = ttk.Button(dc_in_frame, text="تعيين", width=6, command=self.start_recording_dc_inbound)
        self.dc_inbound_record_btn.pack(side='right')

        # Tamper
        tamper_frame = ttk.Frame(frame)
        tamper_frame.pack(fill='x', padx=10, pady=2)
        self.tamper_btn = tk.Button(tamper_frame, text="تلاعب", width=12, bg=self.colors['bg_light'], fg=self.colors['text'],
                                    activebackground=self.colors['warning'], activeforeground='white', bd=0,
                                    command=self.toggle_tamper)
        self.tamper_btn.pack(side='right')
        ttk.Label(tamper_frame, text="الاختصار:").pack(side='right', padx=(0, 10))
        ...
        self.tamper_hotkey_entry = tk.Entry(tamper_frame, textvariable=self.tamper_hotkey_var, width=10, state="readonly",
                                           bd=0, highlightthickness=0, bg=self.colors['bg_light'], fg=self.colors['text'], readonlybackground=self.colors['bg_light'])
        self.tamper_hotkey_entry.pack(side='right', padx=5)
        self.tamper_record_btn = ttk.Button(tamper_frame, text="تعيين", width=6, command=self.start_recording_tamper)
        self.tamper_record_btn.pack(side='right')
        tamper_info = ttk.Label(tamper_frame, text=" (?)", foreground=self.colors['text_dim'], cursor='hand2')
        tamper_info.pack(side='right')
        self._add_tooltip(tamper_info, "إتلاف الحزم بدلاً من إسقاطها.\nقد يسبب سلوكاً غريباً بدلاً من قطع الاتصال الكامل.")
        ...
        ttk.Label(trig_header, text="── نسخ Wolfpack/Triggernade ──", style='Header.TLabel').pack(side='right')
        trig_info = ttk.Label(trig_header, text=" (?)", foreground=self.colors['text_dim'], cursor='hand2')
        trig_info.pack(side='right')
        self._add_tooltip(trig_info, "سجل مسار السحب من فتحة العنصر إلى الأرض.\n\nتأكد من أن المخزون ممتلئ بعناصر لا تريد نسخها.\n\nاضغط Q لإخراج القنبلة ثم اضغط مفتاح التشغيل.")

        trig_hk = ttk.Frame(frame)
        trig_hk.pack(fill='x', padx=10, pady=5)
        ttk.Label(trig_hk, text="الاختصار:").pack(side='right')
        ...
        self.triggernade_hotkey_entry = tk.Entry(trig_hk, textvariable=self.triggernade_hotkey_var, width=15, state="readonly",
                                               bd=0, highlightthickness=0, bg=self.colors['bg_light'], fg=self.colors['text'], readonlybackground=self.colors['bg_light'])
        self.triggernade_hotkey_entry.pack(side='right', padx=5)
        self.triggernade_record_btn = ttk.Button(trig_hk, text="تعيين", width=6, command=self.start_recording_triggernade)
        self.triggernade_record_btn.pack(side='right', padx=5)

        trig_drag_frame = ttk.Frame(frame)
        trig_drag_frame.pack(fill='x', padx=10, pady=2)
        self.trig_drag_btn = ttk.Button(trig_drag_frame, text="تسجيل السحب", width=12, command=self.start_trig_drag_recording)
        self.trig_drag_btn.pack(side='right')
        ...
        if trig_slot and trig_drop:
            self.trig_drag_var.set(f"خانة:{list(trig_slot)} أرض:{list(trig_drop)}")
        else:
            self.trig_drag_var.set("لم يتم التسجيل - انقر تسجيل أولاً")
        ttk.Label(trig_drag_frame, textvariable=self.trig_drag_var, font=("Consolas", 8)).pack(side='right', padx=5)

        self.triggernade_repeat_var = tk.BooleanVar(value=self.config.get("triggernade_repeat", False))
        ttk.Checkbutton(frame, text="تلقائي (تكرار حتى الضغط مرة أخرى)", variable=self.triggernade_repeat_var, command=self.save_settings).pack(anchor='e', padx=10, pady=5)

        self.triggernade_q_spam_var = tk.BooleanVar(value=self.config.get("triggernade_q_spam", False))
        ttk.Checkbutton(frame, text="تكرار Q بين الدورات (إعادة تجهيز)", variable=self.triggernade_q_spam_var, command=self.save_settings).pack(anchor='e', padx=10, pady=2)

        cook_frame = ttk.Frame(frame)
        cook_frame.pack(fill='x', padx=10, pady=2)
        ttk.Label(cook_frame, text="وقت الطهي:", width=20, anchor='e', font=("Segoe UI", 9, "bold")).pack(side='right')
        ...
        cook_slider.pack(side='right', padx=5)
        ...
        ttk.Label(cook_frame, text="مللي ثانية").pack(side='right')

        self.create_slider(frame, "وقت استمرار M2:", "trig_m2_hold", 51, 10, 500, "مللي ثانية")
        self.create_slider(frame, "سرعة السحب:", "trig_drag_speed", 8, 3, 20, "مللي ثانية/خطوة")
        self.create_slider(frame, "تأخير قبل الفصل:", "trig_dc_delay", 10, 0, 200, "مللي ثانية")
        self.create_slider(frame, "رميات M1 أثناء الفصل:", "trig_dc_throws", 10, 1, 30, "")
        self.create_slider(frame, "الوقت بين رميات M1:", "trig_throw_delay", 100, 10, 500, "مللي ثانية")
        self.create_slider(frame, "إعادة الاتصال بعد M1 رقم:", "trig_reconnect_after", 1, 1, 20, "")
        self.create_slider(frame, "انتظار قبل تكرار E:", "wait_before_espam", 0, 0, 2000, "مللي ثانية")
        self.create_slider(frame, "مدة تكرار E:", "espam_duration", 250, 0, 5000, "مللي ثانية")
        self.create_slider(frame, "M1 قبل تداخل E:", "trig_m1_before_interweave", 1, 0, 20, "")
        self.create_slider(frame, "انتظار قبل الدورة التالية:", "wait_before_cycle", 100, 0, 2000, "مللي ثانية")

        ttk.Label(frame, text="─ إعدادات حلقة Wolfpack ─", font=("Segoe UI", 9, "bold")).pack(pady=(10, 5))
        self.create_slider(frame, "استمرار M1 في الحلقة:", "wolfpack_m1_hold", 20, 10, 200, "مللي ثانية")
        self.create_slider(frame, "فجوة M1 في الحلقة:", "wolfpack_m1_gap", 20, 10, 200, "مللي ثانية")
        self.create_slider(frame, "استمرار الفصل في الحلقة:", "wolfpack_dc_hold", 20, 10, 500, "مللي ثانية")
        self.create_slider(frame, "فجوة الفصل في الحلقة:", "wolfpack_dc_gap", 600, 100, 3000, "مللي ثانية")

        trig_btn_frame = ttk.Frame(frame)
        trig_btn_frame.pack(pady=5)
        ttk.Button(trig_btn_frame, text="استعادة الافتراضيات", command=self.reset_triggernade_defaults).pack(side='right', padx=2)
        ttk.Button(trig_btn_frame, text="تصدير", width=7, command=self.export_triggernade).pack(side='right', padx=2)
        ttk.Button(trig_btn_frame, text="استيراد", width=7, command=self.import_triggernade).pack(side='right', padx=2)
        self.triggernade_status_var = tk.StringVar(value="جاهز")
        ...
        ttk.Label(qd_header, text="── رمي بدون فصل (تجريبي) ──", style='Header.TLabel').pack(side='right')
        qd_info = ttk.Label(qd_header, text=" (?)", foreground=self.colors['text_dim'], cursor='hand2')
        qd_info.pack(side='right')
        self._add_tooltip(qd_info, "مثل Triggernade ولكن بدون فصل.\nيستخدم قائمة السياق (النقر بزر الماوس الأيمن) للإسقاط.\nسريع جداً.")

        qd_hk = ttk.Frame(frame)
        qd_hk.pack(fill='x', padx=10, pady=5)
        ttk.Label(qd_hk, text="الاختصار:").pack(side='right')
        ...
        self.quickdrop_hotkey_entry = tk.Entry(qd_hk, textvariable=self.quickdrop_hotkey_var, width=15, state="readonly",
                                         bd=0, highlightthickness=0, bg=self.colors['bg_light'], fg=self.colors['text'], readonlybackground=self.colors['bg_light'])
        self.quickdrop_hotkey_entry.pack(side='right', padx=5)
        self.quickdrop_record_btn = ttk.Button(qd_hk, text="تعيين", width=6, command=self.start_recording_quickdrop)
        self.quickdrop_record_btn.pack(side='right', padx=5)

        qd_pos_frame = ttk.Frame(frame)
        qd_pos_frame.pack(fill='x', padx=10, pady=2)
        self.quickdrop_pos_btn = ttk.Button(qd_pos_frame, text="تسجيل الموقع", width=12, command=self.start_quickdrop_pos_recording)
        self.quickdrop_pos_btn.pack(side='right')
        ...
        self.quickdrop_pos_var.set(f"يمين:{qd_rclick} يسار:{qd_lclick}")
        ttk.Label(qd_pos_frame, textvariable=self.quickdrop_pos_var, font=("Consolas", 8)).pack(side='right', padx=5)

        self.quickdrop_repeat_var = tk.BooleanVar(value=self.config.get("quickdrop_repeat", False))
        ttk.Checkbutton(frame, text="تلقائي (تكرار)", variable=self.quickdrop_repeat_var, command=self.save_settings).pack(anchor='e', padx=10, pady=5)

        self.create_slider(frame, "وقت الطهي:", "quickdrop_cook", 1000, 100, 3000, "مللي ثانية")
        self.create_slider(frame, "تأخير المخزون:", "quickdrop_inv_delay", 50, 10, 500, "مللي ثانية")
        self.create_slider(frame, "تأخير القائمة:", "quickdrop_menu_delay", 20, 10, 200, "مللي ثانية")
        self.create_slider(frame, "تأخير الإسقاط:", "quickdrop_drop_delay", 20, 10, 200, "مللي ثانية")

        self.quickdrop_status_var = tk.StringVar(value="جاهز")
        ...
        ttk.Label(mine_header, text="── نسخ الألغام ──", style='Header.TLabel').pack(side='right')
        mine_info = ttk.Label(mine_header, text=" (?)", foreground=self.colors['text_dim'], cursor='hand2')
        mine_info.pack(side='right')
        self._add_tooltip(mine_info, "سجل سحب اللغم من الخانة إلى الأرض.\nاضبط وقت الطهي والتأخير.")

        mine_hk = ttk.Frame(frame)
        mine_hk.pack(fill='x', padx=10, pady=5)
        ttk.Label(mine_hk, text="الاختصار:").pack(side='right')
        ...
        self.mine_hotkey_entry = tk.Entry(mine_hk, textvariable=self.mine_hotkey_var, width=15, state="readonly",
                                         bd=0, highlightthickness=0, bg=self.colors['bg_light'], fg=self.colors['text'], readonlybackground=self.colors['bg_light'])
        self.mine_hotkey_entry.pack(side='right', padx=5)
        self.mine_record_btn = ttk.Button(mine_hk, text="تعيين", width=6, command=self.start_recording_mine)
        self.mine_record_btn.pack(side='right', padx=5)

        self.mine_repeat_var = tk.BooleanVar(value=self.config.get("mine_repeat", False))
        ttk.Checkbutton(frame, text="تكرار تلقائي", variable=self.mine_repeat_var, command=self.save_settings).pack(anchor='e', padx=10, pady=2)

        mine_drag_frame = ttk.Frame(frame)
        mine_drag_frame.pack(fill='x', padx=10, pady=5)
        self.mine_drag_btn = ttk.Button(mine_drag_frame, text="تسجيل السحب", width=12, command=self.start_mine_drag_recording)
        self.mine_drag_btn.pack(side='right')
        ...
        self.mine_drag_var.set(f"خانة:{mine_slot} أرض:{mine_drop}")
        ttk.Label(mine_drag_frame, textvariable=self.mine_drag_var, font=("Consolas", 8)).pack(side='right', padx=10)

        ttk.Label(frame, text="التوقيتات:", font=("Segoe UI", 9, "bold")).pack(anchor='e', padx=10, pady=(5, 2))

        mine_cook_frame = ttk.Frame(frame)
        mine_cook_frame.pack(fill='x', padx=10, pady=2)
        ttk.Label(mine_cook_frame, text="وقت الطهي:", width=20, anchor='e', font=("Segoe UI", 9, "bold")).pack(side='right')
        ...
        ttk.Label(mine_cook_frame, text="مللي ثانية").pack(side='right')

        self.create_slider(frame, "تأخير قبل الفصل:", "mine_dc_delay", 99, 0, 500, "مللي ثانية", bold=True)
        self.create_slider(frame, "سرعة السحب:", "mine_drag_speed", 8, 3, 20, "مللي ثانية")
        self.create_slider(frame, "تأخير قبل الإغلاق:", "mine_pre_close", 100, 0, 2000, "مللي ثانية")
        self.create_slider(frame, "استمرار TAB (إغلاق):", "mine_tab_hold", 80, 20, 300, "مللي ثانية")
        self.create_slider(frame, "الإغلاق لإعادة الاتصال:", "mine_close_reconnect", 409, 100, 2000, "مللي ثانية", bold=True)
        self.create_slider(frame, "إعادة الاتصال للنقر:", "mine_click_delay", 7, 0, 500, "مللي ثانية", bold=True)
        self.create_slider(frame, "استمرار نقر النسخ:", "mine_pickup_hold", 1336, 100, 3000, "مللي ثانية", bold=True)
        self.create_slider(frame, "تأخير قبل E:", "mine_e_delay", 868, 0, 2000, "مللي ثانية")
        self.create_slider(frame, "تأخير قبل الحلقة:", "mine_loop_delay", 550, 0, 2000, "مللي ثانية")
        ...
        ttk.Checkbutton(reselect_frame, text="Q لإعادة التحديد", variable=self.mine_reselect_var, command=self.save_settings).pack(side='right')

        self.mine_q_mode_var = tk.StringVar(value=self.config.get("mine_q_mode", "radial"))
        ttk.Radiobutton(reselect_frame, text="نقر بسيط", variable=self.mine_q_mode_var, value="simple", command=self.save_settings).pack(side='right', padx=(10, 5))
        ttk.Radiobutton(reselect_frame, text="دائري:", variable=self.mine_q_mode_var, value="radial", command=self.save_settings).pack(side='right')
        ...
        self.mine_q_dir_btn.pack(side='right', padx=2)
        ...
        ttk.Checkbutton(nudge_frame, text="تحريك الماوس", variable=self.mine_nudge_var, command=self.save_settings).pack(side='right')
        ...
        nudge_entry.pack(side='right', padx=5)
        nudge_entry.bind('<Return>', lambda e: self.save_settings())
        nudge_entry.bind('<FocusOut>', lambda e: self.save_settings())
        ttk.Label(nudge_frame, text="بكسل يمين لكل حلقة").pack(side='right')

        mine_btn_frame = ttk.Frame(frame)
        mine_btn_frame.pack(pady=5)
        ttk.Button(mine_btn_frame, text="استعادة الافتراضيات", command=self.reset_mine_defaults).pack(side='right', padx=2)
        ttk.Button(mine_btn_frame, text="تصدير", width=7, command=self.export_mine).pack(side='right', padx=2)
        ttk.Button(mine_btn_frame, text="استيراد", width=7, command=self.import_mine).pack(side='right', padx=2)
        self.mine_status_var = tk.StringVar(value="جاهز")
        ...
        ttk.Label(custom_macro_header, text="── ماكرو مخصص ──", style='Header.TLabel').pack(side='right')
        custom_macro_info = ttk.Label(custom_macro_header, text=" (?)", foreground=self.colors['text_dim'], cursor='hand2')
        custom_macro_info.pack(side='right')
        self._add_tooltip(custom_macro_info, "تسجيل وتشغيل وحدات ماكرو مخصصة.\nCtrl+Enter لبدء/إيقاف التسجيل.\nيمكن تسجيل أي مفتاح بما في ذلك ESC.")

        # حاوية أزرار التبويب
        self.macro_tabs_container = ttk.Frame(frame)
        self.macro_tabs_container.pack(fill='x', padx=10, pady=5)

        # زر التمرير يمين (يظهر المحتوى المخفي يميناً - أي يرجع للخلف)
        ...
        # زر التمرير يسار (يظهر المحتوى المخفي يساراً - أي يتقدم)
        ...
        self.macro_add_btn.pack(side='right', padx=2)
        self.macro_scroll_right.pack(side='right')
        self.macro_tabs_canvas.pack(side='right', fill='x', expand=True)
        self.macro_scroll_left.pack(side='right')

        # Frame inside canvas for tab buttons
        self.macro_tabs_frame = ttk.Frame(self.macro_tabs_canvas)
        self.macro_tabs_window = self.macro_tabs_canvas.create_window((0, 0), window=self.macro_tabs_frame, anchor='ne')
        ...
        ttk.Label(name_frame, text="الاسم:").pack(side='right')
        self.macro_name_var = tk.StringVar()
        self.macro_name_entry = tk.Entry(name_frame, textvariable=self.macro_name_var, width=20, justify='right',
        ...
        self.macro_name_entry.pack(side='right', padx=5)
        ...
        ttk.Label(hotkey_frame, text="الاختصار:").pack(side='right')
        self.macro_hotkey_var = tk.StringVar()
        self.macro_hotkey_entry = tk.Entry(hotkey_frame, textvariable=self.macro_hotkey_var, width=15, state="readonly", justify='right',
                                           bd=0, highlightthickness=0, bg=self.colors['bg_light'], fg=self.colors['text'],
                                           readonlybackground=self.colors['bg_light'])
        self.macro_hotkey_entry.pack(side='right', padx=5)
        self.macro_hk_btn = ttk.Button(hotkey_frame, text="تعيين", width=6, command=self._start_recording_macro_hotkey)
        self.macro_hk_btn.pack(side='right', padx=5)

        # Speed slider row
        speed_frame = ttk.Frame(self.macro_content_frame)
        speed_frame.pack(fill='x', pady=2)
        ttk.Label(speed_frame, text="السرعة:").pack(side='right')
        ...
        self.macro_speed_slider.pack(side='right', padx=5)
        self.macro_speed_label = ttk.Label(speed_frame, text="1.0x", width=5)
        self.macro_speed_label.pack(side='right')

        # Keep Timing checkbox
        self.macro_keep_timing_var = tk.BooleanVar(value=False)
        ttk.Checkbutton(speed_frame, text="الحفاظ على التوقيت", variable=self.macro_keep_timing_var,
                        command=self._on_macro_keep_timing_change).pack(side='right', padx=10)
        ...
        ttk.Checkbutton(repeat_frame, text="تكرار", variable=self.macro_repeat_var,
                        command=self._on_macro_repeat_change).pack(side='right')
        ...
        self.macro_repeat_times_entry.pack(side='right', padx=2)
        # Only allow integers
        self.macro_repeat_times_entry.bind('<KeyRelease>', self._validate_repeat_times)
        ttk.Label(repeat_frame, text="مرات").pack(side='right')
        ...
        ttk.Checkbutton(repeat_frame, text="∞", variable=self.macro_repeat_infinite_var,
                        command=self._on_macro_infinite_change).pack(side='right', padx=5)

        ttk.Label(repeat_frame, text="تأخير:").pack(side='right', padx=(10, 0))
        ...
        self.macro_repeat_delay_entry = tk.Entry(repeat_frame, textvariable=self.macro_repeat_delay_var, width=4,
                                                  bd=0, highlightthickness=1, bg=self.colors['bg_light'],
                                                  fg=self.colors['text'], justify='center')
        self.macro_repeat_delay_entry.pack(side='right', padx=2)
        self.macro_repeat_delay_entry.bind('<KeyRelease>', self._validate_repeat_delay)
        ttk.Label(repeat_frame, text="ث").pack(side='right')

        # Buttons row
        macro_btn_frame = ttk.Frame(self.macro_content_frame)
        macro_btn_frame.pack(pady=2)
        self.macro_record_btn = ttk.Button(macro_btn_frame, text="تسجيل", width=8, command=self.start_custom_macro_recording)
        self.macro_record_btn.pack(side='right', padx=2)
        self.macro_play_btn = ttk.Button(macro_btn_frame, text="تشغيل", width=8, command=self._toggle_macro_play)
        self.macro_play_btn.pack(side='right', padx=2)
        self.macro_delete_btn = ttk.Button(macro_btn_frame, text="حذف", width=6, command=self._delete_current_macro)
        self.macro_delete_btn.pack(side='right', padx=2)

        # Export/Import row
        macro_io_frame = ttk.Frame(self.macro_content_frame)
        macro_io_frame.pack(pady=2)
        ttk.Button(macro_io_frame, text="تصدير", width=8, command=self._export_current_macro).pack(side='right', padx=2)
        ttk.Button(macro_io_frame, text="استيراد", width=8, command=self._import_macro).pack(side='right', padx=2)

        # Status label
        self.macro_status_var = tk.StringVar(value="Ctrl+Enter لبدء التسجيل")
        ...
        self.espam_record_btn = ttk.Button(espam_hk, text="تعيين", width=6, command=self.start_recording_espam)
        ...
        self.minimize_record_btn = ttk.Button(min_hk_frame, text="تعيين", width=4, command=self.start_recording_minimize)
        ...
        self.tray_record_btn = ttk.Button(tray_hk_frame, text="تعيين", width=4, command=self.start_recording_tray)
        ...
        self.stop_record_btn = ttk.Button(stop_frame, text="تعيين", width=6, command=self.start_recording_stop)
    ...
    def create_slider(self, parent, label, config_key, default, min_val, max_val, unit, bold=False, tooltip=None):
        """Create a slider row with label, slider, and editable value entry (RTL)"""
        row = ttk.Frame(parent)
        row.pack(fill='x', padx=10, pady=2)

        if bold:
            lbl = ttk.Label(row, text=label, width=20, anchor='e', font=('Segoe UI', 9, 'bold'))
        else:
            lbl = ttk.Label(row, text=label, width=20, anchor='e')
        lbl.pack(side='right')
        ...
        if tooltip:
            def show_tooltip(event):
                ...
                tip_label = tk.Label(tip, text=tooltip, justify='right', background="#ffffe0",
    ...
    def start_recording_mine(self):
        ...
        self.mine_hotkey_var.set("اضغط مفتاح...")
    ...
    def _show_direction_picker(self):
        ...
        title_label = tk.Label(popup, text="اختر الخانة", bg='white', fg='black', font=('Arial', 12, 'bold'), padx=10, pady=5)
    ...
    def _build_macro_tabs(self):
        ...
        for i, macro in enumerate(macros):
            ...
            name = macro.get("name", f"ماكرو {i+1}")
            if is_active:
    ...
    def _update_macro_tabs_scroll(self, event=None):
        ...
        if hasattr(self, 'macro_tabs_canvas'):
            ...
            if content_width > canvas_width and canvas_width > 1:
                self.macro_scroll_right.pack(side='right', before=self.macro_tabs_canvas)
                self.macro_scroll_left.pack(side='right', after=self.macro_tabs_canvas, before=self.macro_add_btn)
            else:
    ...
    def _delete_current_macro(self):
        ...
        macros = self.custom_macros_data.get("macros", [])
        if len(macros) <= 1:
            self.show_overlay("لا يمكن حذف آخر ماكرو", force=True)
            return
    ...
    def _export_current_macro(self):
        ...
        if not macro.get("events"):
            self.show_overlay("لا يوجد تسجيل للتصدير", force=True)
            return
        ...
        if path:
            ...
            self.show_overlay(f"تم التصدير: {name}", force=True)
            ...
            try:
                ...
                if "events" not in imported:
                    ...
                    if isinstance(imported, list):
                        imported = {"name": "مستورد", "hotkey": "", "speed": 1.0, "events": imported}
                    else:
                        self.show_overlay("ملف ماكرو غير صالح", force=True)
                        return

                # Add as new macro
                macros = self.custom_macros_data.get("macros", [])
                # Ensure unique name
                base_name = imported.get("name", "مستورد")
                name = base_name
                counter = 1
                ...
                self._load_current_macro_to_ui()
                self.register_hotkeys()
                self.show_overlay(f"تم الاستيراد: {name}", force=True)
                print(f"[MACRO] Imported '{name}' from {path}")
            except Exception as e:
                self.show_overlay("فشل الاستيراد", force=True)
                print(f"[MACRO] فشل الاستيراد: {e}")
    ...
    def _get_current_macro(self):
        ...
        return {"name": "ماكرو 1", "hotkey": "", "speed": 1.0, "events": []}
    ...
    def _start_recording_macro_hotkey(self):
        ...
        self.macro_hotkey_var.set("اضغط مفتاح...")
    ...
    def _on_macro_hotkey_press(self, event):
        ...
        if key in ('escape', 'esc'):
            ...
            self.macro_hk_btn.config(text="تعيين")
    ...
    def start_custom_macro_recording(self):
        ...
        self.macro_status_var.set("اضغط Ctrl+Enter للبدء")
        self.show_overlay("اضغط Ctrl+Enter للبدء", force=True)
        ...
        def on_key_press(key):
            ...
            if key == kb.Key.enter and self._ctrl_held:
                if not self._custom_macro_recording_active:
                    ...
                    self.root.after(0, lambda: self.macro_record_btn.config(text="جاري التسجيل..."))
                    self.root.after(0, lambda: self.macro_status_var.set("جاري التسجيل... اضغط Ctrl+Enter للإيقاف"))
                    self.root.after(0, lambda: self.show_overlay("جاري التسجيل... اضغط Ctrl+Enter للإيقاف", force=True))
                else:
                    # Stop recording
                    mouse_listener.stop()
    ...
    def _save_custom_macro_recording(self):
        ...
        if len(self._custom_macro_recording) < 2:
            self.macro_status_var.set("التسجيل قصير جداً")
            self.macro_record_btn.config(text="تسجيل")
            self.show_overlay("تم إلغاء التسجيل - قصير جداً", force=True)
            return
        ...
        self.macro_status_var.set(f"تم حفظ: {num_events} أحداث")
        self.macro_record_btn.config(text="تسجيل")
        self.show_overlay(f"تم حفظ {num_events} أحداث", force=True)
    ...
    def play_custom_macro(self, macro_index=None):
        ...
        if not recording:
            self.show_overlay("لا يوجد تسجيل للتشغيل", force=True)
            return
        ...
        def on_esc():
            if not self._macro_stop:
                self._macro_stop = True
                self.root.after(0, lambda: self.macro_play_btn.config(text="تشغيل"))
                self.root.after(0, lambda: self.macro_status_var.set("تم الإلغاء"))
                self.root.after(0, lambda: self.show_overlay("تم إلغاء الماكرو", force=True))
        self._macro_esc_hook = keyboard.on_press_key('esc', lambda e: on_esc(), suppress=False)

        self.macro_play_btn.config(text="إيقاف")
        self.macro_status_var.set("تشغيل الماكرو... (ESC للإلغاء)")
    ...
    def start_mine_drag_recording(self):
        ...
        self.mine_drag_btn.config(text="جاري التسجيل...")
        self.show_overlay("اسحب العنصر إلى الأرض", force=True)
        ...
        def on_click(x, y, button, pressed):
            ...
            if pressed:
                ...
                self.root.after(0, lambda: self.show_overlay("الآن اترك...", force=True))
                ...
                if duration_ms < 20 or distance < 50:
                    ...
                    self._mine_drag_started = False
                    self.root.after(0, lambda: self.show_overlay("اسحب العنصر إلى الأرض", force=True))
                    return
    ...
    def start_recording_espam(self):
        ...
        self.espam_hotkey_var.set("اضغط مفتاح...")
        self.root.bind("<KeyPress>", self.on_key_press)
        self.root.focus_force()
    ...
    def start_recording_triggernade(self):
        ...
        self.triggernade_hotkey_var.set("اضغط مفتاح...")
        self.root.bind("<KeyPress>", self.on_key_press)
        self.root.focus_force()
    ...
    def start_recording_quickdrop(self):
        ...
        self.quickdrop_hotkey_var.set("اضغط مفتاح...")
        self.root.bind("<KeyPress>", self.on_key_press)
        self.root.focus_force()
    ...
    def start_recording_stop(self):
        ...
        self.stop_hotkey_var.set("اضغط مفتاح...")
        self.root.bind("<KeyPress>", self.on_key_press)
        self.root.focus_force()
    ...
    def _pick_bg_color(self):
        ...
        color = colorchooser.askcolor(color=self.bg_color_var.get(), title="اختر لون الخلفية")
        if color[1]:
    ...
    def _pick_fg_color(self):
        ...
        from tkinter import colorchooser
        color = colorchooser.askcolor(color=self.fg_color_var.get(), title="اختر لون النص")
        if color[1]:
    ...
    def _pick_accent_color(self):
        ...
        from tkinter import colorchooser
        color = colorchooser.askcolor(color=self.accent_color_var.get(), title="اختر لون التمييز")
        if color[1]:
    ...
    def _reset_dc_buttons(self):
        ...
        self.dc_both_btn.config(text="فصل الكل", bg=self.colors['bg_light'])
        self.dc_outbound_btn.config(text="فصل الصادر", bg=self.colors['bg_light'])
        self.dc_inbound_btn.config(text="فصل الوارد", bg=self.colors['bg_light'])
    ...
    def toggle_dc_both(self):
        ...
        if is_dropping():
            ...
            self.root.after(0, lambda: self.show_overlay("تم إعادة الاتصال"))
        else:
            start_packet_drop(outbound=True, inbound=True)
            self._reset_dc_buttons()
            self.dc_both_btn.config(text="إعادة الاتصال", bg=self.colors['highlight'])
            self.root.after(0, lambda: self.show_overlay("فصل الكل"))
    ...
    def toggle_dc_outbound(self):
        ...
        if is_dropping():
            ...
            self._reset_dc_buttons()
            self.dc_outbound_btn.config(text="إعادة الاتصال", bg=self.colors['highlight'])
            self.root.after(0, lambda: self.show_overlay("فصل الصادر"))
    ...
    def toggle_dc_inbound(self):
        ...
        if is_dropping():
            ...
            self._reset_dc_buttons()
            self.dc_inbound_btn.config(text="إعادة الاتصال", bg=self.colors['highlight'])
            self.root.after(0, lambda: self.show_overlay("فصل الوارد"))

    def toggle_tamper(self):
        """Toggle packet tampering - corrupts packets but still sends them"""
        if self.tampering:
            stop_packet_tamper()
            self.tamper_btn.config(text="تلاعب", bg=self.colors['bg_light'])
            self.tampering = False
            self.root.after(0, lambda: self.show_overlay("تلاعب متوقف"))
        else:
            start_packet_tamper(outbound=True, inbound=True)
            self.tamper_btn.config(text="إيقاف التلاعب", bg=self.colors['warning'])
            self.tampering = True
            self.root.after(0, lambda: self.show_overlay("تلاعب مفعل"))
    ...
    def start_recording_minimize(self):
        ...
        self.minimize_hotkey_var.set("اضغط مفتاح...")
        self.root.bind("<KeyPress>", self.on_key_press)
        self.root.focus_force()
    ...
    def start_recording_tray(self):
        ...
        self.tray_hotkey_var.set("اضغط مفتاح...")
        self.root.bind("<KeyPress>", self.on_key_press)
        self.root.focus_force()
    ...
    def reset_all_settings(self):
        ...
        if not mb.askyesno("إعادة تعيين جميع الإعدادات", "سيقوم هذا بإعادة تعيين كل شيء بما في ذلك:\n\n• جميع مفاتيح الاختصار\n• جميع إعدادات التوقيت\n• جميع المواقع المسجلة\n• تفضيلات السحب والإفلات\n\nهل أنت متأكد؟"):
            return
        ...
        self.show_overlay("تم إعادة تعيين جميع الإعدادات!")
    ...
    def _set_triggernade_settings(self, data):
        ...
        if self.trig_slot_pos and self.trig_drop_pos:
            self.trig_drag_var.set(f"خانة:{list(self.trig_slot_pos)} أرض:{list(self.trig_drop_pos)}")
    ...
    def _set_mine_settings(self, data):
        ...
        if "mine_drop_pos" in data:
            ...
            self.mine_drag_var.set(f"خانة:{list(self.mine_slot_pos)} أرض:{list(self.mine_drop_pos)}")
        self.save_settings()
    ...
    def register_hotkeys(self):
        ...
        try:
            ...
            print(f"[HOTKEY] خطأ في مسح الاختصارات: {e}")
        ...
        if trig_hk and trig_hk != "اضغط مفتاح...":
            try:
                ...
                print(f"[HOTKEY] فشل triggernade '{trig_hk}': {e}")

        qd_hk = self.quickdrop_hotkey_var.get()
        if qd_hk and qd_hk != "اضغط مفتاح...":
            try:
                ...
                print(f"[HOTKEY] فشل quickdrop '{qd_hk}': {e}")

        if espam_hk and espam_hk != "اضغط مفتاح...":
            try:
                ...
                print(f"[HOTKEY] فشل espam '{espam_hk}': {e}")

        mine_hk = self.mine_hotkey_var.get()
        if mine_hk and mine_hk != "اضغط مفتاح...":
            try:
        ...
        if dc_both_hk and dc_both_hk != "اضغط مفتاح..." and dc_both_hk != "...":
            try:
        ...
        if dc_outbound_hk and dc_outbound_hk != "اضغط مفتاح..." and dc_outbound_hk != "...":
            try:
        ...
        if dc_inbound_hk and dc_inbound_hk != "اضغط مفتاح..." and dc_inbound_hk != "...":
            try:
        ...
        if tamper_hk and tamper_hk != "اضغط مفتاح..." and tamper_hk != "...":
            try:
        ...
        if min_hk and min_hk != "اضغط مفتاح...":
            try:
        ...
        if tray_hk and tray_hk != "اضغط مفتاح...":
            try:
        ...
        if stop_hk and stop_hk != "اضغط مفتاح...":
            try:
    ...
    def on_close(self):
        ...
        try:
            ...
            print(f"[CLOSE] خطأ في استعادة الشبكة: {e}")
